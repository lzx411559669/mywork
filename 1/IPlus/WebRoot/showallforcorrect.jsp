                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover">
                                    ...
                                    <c:forEach items="${list}" var="test">
				                      <tr>
				                      	   <td>${test.fcno}</td>
										   <td>${test.sj.suname}</td>
										   <td>${test.xztmark}</td>
				                           <td>${test.testsno}</td>
										   <td>${test.stu.sname}</td>
										   <td>
										   <c:if test="${test.condition==1}">
				                           	<i class="text-warning">未批阅</i>
   											</c:if>
   											<c:if test="${test.condition==2}">
				                           	<i class="text-success">已批阅</i>
   											</c:if>
										   </td>
				                           <td>
				                           <c:if test="${test.condition==1}">
										   <a type="button" class="btn btn-danger" href="javascript:if(confirm('是否开始批改试卷'))location='correct.do?correct&fcno=${test.fcno}&tid=${t.tid}'">
				                         	  开始阅卷
				                           </a>
				                           </c:if> 
				                           <c:if test="${test.condition==2}">
										   <a type="button" class="btn btn-danger" href="javascript:if(confirm('是否删除该已批阅试卷'))location='correct.do?del&fcno=${test.fcno}'">
				                         	 删除该卷
				                           </a>
				                           </c:if>                           
										   </td>               
                                ...
                                <div style="margin-left: 1000px">
		                           <a  href="correct.do?show&index=1">第一页</a>
		                           <c:if test="${total>index}">
		                           <a href="correct.do?show&index=${index+1}">下一页</a>
		                           </c:if>
		                           <c:if test="${index>1}">
		                           <a href="correct.do?show&index=${index-1}">上一页</a>
		                           </c:if>
		                           <a href="correct.do?show&index=${total}">最后一页</a>
		                       </div>
		                       <!-- 翻页 -->
                            </div>
                        </div>
